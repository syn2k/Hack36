...
###########-------- STEP 2 - Removing top, bottom, left and right white spaces from photograph ------------#############
  ...
  # print(pixels.shape)
  while np.average(pixels[0])>240:
    pixels = np.delete(pixels, 0, 0) #delete top white space
  while np.average(pixels[pixels.shape[0]-1])>240:
...
def remove_space_top_bottom(i):
  ...
  # print(pixels.shape)
  while np.average(i[0])>240:
    i = np.delete(i, 0, 0) #delete top white space
  while np.average(i[i.shape[0]-1])>240:
...
def get_value(imgc):
  ...
  for i in all_digits:
    ...
    h2=18
    w2=h2*w/h
    top=int(math.ceil(((28-h2)*h)/(2*h2)))
    left=  int(math.ceil(((28-w2)*w)/(2*w2)))
    # print(top,left)
